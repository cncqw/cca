...
**情境：** 调用 `get_customer` 和 `lookup_order` 后，智能体拥有所有可用的系统数据，但仍面临不确定性。哪种情况是调用 `escalate_to_human` 最合理的触发器？
...
**情境：** 生产指标显示你的智能体每次解决平均超过4次API循环。分析显示Claude经常在即使最初两者都需要时，也在单独的顺序轮次中请求 `get_customer` 和 `lookup_order`。减少循环次数最有效的方法是什么？
